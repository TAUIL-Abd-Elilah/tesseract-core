...
class PortInUseError(RuntimeError):
    ...
    - host networking: the container binds the host port directly, so the
      ...
      use" and is detected in ``wait_for_health``.
    """
...
def wait_for_health(served: ServedTesseract, url: str, timeout: float) -> None:
    ...
    try:
        ...
        except (APIError, OSError, subprocess.CalledProcessError) as ex:
